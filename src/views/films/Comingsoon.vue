    ...
    <div class="comingsoon">
        <ul>
            ...
            <img :src="data.poster" alt="">
                <div class="info">
                    <h4 class="name">{{data.name}} <span class="item">{{data.filmType.name}}</span></h4>
                   
                    <p>观众评分：<span class="grade">{{data.grade}}</span></p>
                    <p class="actors">主演：{{data.actors | actorFilter}}</p>
                    <span class="address">{{data.nation| runtime}}</span>
                 </div>
...
export default {
    ...
    created() {
        instance.get(`gateway?cityId=${this.cityId}310100&pageNum=1&pageSize=10&type=2&k=2021284`,{
            headers:{
